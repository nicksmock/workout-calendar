  ...
  const saveWorkoutData = async (week: number, day: number, data: Partial<WorkoutSession>) => {
    ...
    try {
      ...
      if (existingSession?.id) {
        ...
        updatedSession = await workoutApi.updateSession(existingSession.id, {
          ...data,
  ...
  const calculateStats = (currentWeek: number) => {
    ...
    weeks.forEach(week => {
      for (let day = 0; day < 7; day++) {
        ...
        // Add pushups and plank data if available
        if (data.pushups) pushupProgress.push(data.pushups);
        if (data.plank_hold) plankProgress.push(data.plank_hold);
      }
    });
